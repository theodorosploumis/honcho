...
This starts four services: **api** (host port 8000 by default), **deriver** (background worker), **database** (PostgreSQL with pgvector, host port 5433 by default), and **redis** (host port 6380 by default). The published ports are bound to `127.0.0.1`; the services continue to use their internal ports 8000, 5432, and 6379. Redis caching is enabled by default. Set `API_PORT`, `POSTGRES_PORT`, or `REDIS_PORT` in `.env` to use different host ports.
