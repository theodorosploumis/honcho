...
This starts four services: **api** (port 8000), **deriver** (background worker), **database** (PostgreSQL with pgvector, port 5432 by default), and **redis** (port 6379 by default). All ports are bound to `127.0.0.1`. Redis caching is enabled by default. Set `POSTGRES_PORT` or `REDIS_PORT` in `.env` to use different host ports for PostgreSQL or Redis.
