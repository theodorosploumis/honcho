...
This starts four services: **api** (port 8000), **deriver** (background worker), **database** (PostgreSQL with pgvector, port 5432 by default), and **redis** (port 6379). All ports are bound to `127.0.0.1`. Redis caching is enabled by default. Set `POSTGRES_PORT` in `.env` to use a different host port for PostgreSQL.
